  ...
  FsForm = function (){
    ...
    var thisForm = this;
		$(thisForm.config.elem).find("input.fsDate").each(function(){
          ...
          ////cts添加
		  //options["btns"] = ['clear', 'now', 'confirm'];
  ...
  */
  ...
  FsForm.prototype.radioDataRender = function(_this,labelField,valueField,list){
    ...
    var thisForm = this;
		...
		var isUrlParamRender = formDom.attr("isUrlParamRender");//cts 添加isUrlParamRender 不渲染url的值
		if (isUrlParamRender) {
		    return;
		}
   ...
   */
	FsForm.prototype.submitForm = function (param, _this, formElem) {
	 if (_this.attr("noSubmit")) {
	        return;
	 }
	var url = _this.attr("url");//请求url
	var noUrl = fsCommon.noUrl;//不需要请求的url和function验证cts添加
	var funcNo = _this.attr("funcNo");
	FsForm.prototype.isRefreshTable = _this.attr("isRefreshTable");//是否刷新Table
	FsForm.prototype.topRedirectUrl = _this.attr("topRedirectUrl");//跳转地址
	FsForm.prototype.redirectUrl = _this.attr("redirectUrl");//跳转地址
	FsForm.prototype.callJs = _this.attr("callJs");//回调js方法名
	if (!noUrl) {
