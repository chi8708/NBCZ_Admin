...
layui.define(['layer',"fsCommon",'fsConfig'], function(exports){
    ...
    var domTree = $("#"+_this.config.id);
		...
		_this.config.selectNode = domTree.attr("selectNode");//选中节点
    ...
    //选中节点
	FsTree.prototype.selectNode = function (treeId) {
	    var _this = this;
	    if ($.isEmpty(treeId)) {
	        treeId = _this.config.selectNode;
	    }
	    ...
	    if (treeId) {
	        ...
	        if (treeId.indexOf(',')>0) {
	            ...
	            for (var i = 0; i < arr.length; i++) {
	                var node3 = zTree.getNodeByParam(_this.config.treeIdKey, arr[i], null);
	        ...
	        // _this.refresh(_this.config.selectNode);
	    }

	}

	FsTree.prototype.test = function () {
	    alert("aaaa");
	}
