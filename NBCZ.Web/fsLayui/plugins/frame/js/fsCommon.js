 ...
 * @License：MI
 */
//cts 添加button 提交属性：noUrl（不对url和function验证） ,redirectUrl(跳转) ,topRedirectUrl（top跳转），callJs(执行成功回调)
    ...
    invoke : function (url,param,callBackFunc,async,method)
    {
      ...
      $.ajax({
        ...
        success: function(result){
          ...
          if (result[statusName] == successNo) {
              if (fsCommon.redirectUrl) {
                  location.href = fsCommon.redirectUrl
                  fsCommon.redirectUrl = null;
                  return;
              }
              if (fsCommon.topRedirectUrl) {
                  parent.frames[0].location.href = fsCommon.topRedirectUrl;
                  fsCommon.topRedirectUrl = null;
                  return;
              }
          ...
          if (fsCommon.callJs) {
              ...
              fsCommon.callJs = null;
              var r = fn.call(this, result);
              if (r===false) {
                  return;
              }
          }
...
//      	 console.error(errorThrown);
    ...
    buttonEvent : function (fsType,getDatagrid)
    {
    	...
    	botton.on("click",function(event){
    	    var _this = $(this);
    	    fsCommon.noUrl = _this.attr("noUrl");//不需要请求的url和function验证cts添加
    	    fsCommon.topRedirectUrl = _this.attr("topRedirectUrl");//跳转地址
    	    fsCommon.redirectUrl = _this.attr("redirectUrl");//跳转地址
    	    fsCommon.callJs = _this.attr("callJs");//回调js方法名
