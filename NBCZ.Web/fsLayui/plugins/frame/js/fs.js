 ...
 */
layui.config({
    base: "/fsLayui/plugins/frame/js/",//设定扩展的Layui模块的所在目录，一般用于外部模块扩展 虚拟目录时需要调整如果不是绝对路径报错Cannot read property '$' of null
	version : '1.9.0'
});
...
layui.fsUtil.toDateString = function(d, format){
 ...
 var date = new Date(d.replace(/-/g, "/") || new Date())//iebug cts 修复
...
//cts添加 重新加载iframe内容
function reloadFrame(sourceName,url) {
    var frame = getFrame(sourceName);
    frame.location.href = url;
}

//cts添加 获取加载的iframe
function getFrame(sourceName) {
    var frames = window.parent.frames;
    for (var i = 0; i < frames.length; i++) {
        var frame = frames[i];
        if (frame.location.href.toUpperCase().indexOf(sourceName.toUpperCase()) > 0) {
            return frame;
            break;
        };
    }
