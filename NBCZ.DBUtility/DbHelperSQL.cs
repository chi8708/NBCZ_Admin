...
namespace NBCZ.DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        /// <summary>
        /// 执行SQL 查询语句，返回符合条件的记录条数
        /// </summary>
        /// <param name="sqlString"></param>
        /// <returns></returns>
        public static int Query_int(string sqlString)
        {
            int count;
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
      ...
      #region ext扩展非动态生成
        ...
        public static void ExecuteSqlTran(Dictionary<string,SqlParameter[]> SQLStringList,List<string> proName=null)
        {
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                       // var dic = SQLStringList.OrderBy(p => p.Key);
                        var dic = SQLStringList;
                        foreach (var myDE in dic)
                        {
                            string cmdText = myDE.Key;
                            if (proName != null&&proName.Count>0 && proName.Contains(cmdText))
                            {
